...
class OCBCExtractor(BaseExtractor):
    ...
    def _extract_top_up(self, email: str) -> Optional[TransactionData]:
        """
        Extract details from the top-up email format.
        """
        # Match sender details
        from_match = re.search(r"FROM\s*:\s*(.+)", email)
        # Match recipient details
        to_match = re.search(r"TO\s*:\s*(.+)\n([\d\s]+)", email, re.MULTILINE)
        # Match amount
        amount_match = re.search(r"PAYMENT AMOUNT\s*:\s*IDR\s([\d,]+)", email)
        # Match date
        date_match = re.search(r"PAYMENT DATE\s*:\s*(\d{2} \w+ \d{4} \d{2}:\d{2}:\d{2} WIB)", email)
        # Match reference number
        ref_match = re.search(r"Reference Number:\s*(\w+)", email)
        ...
        if from_match and to_match and amount_match and date_match and ref_match:
            ...
            trx.sender_account = ""  # Not available in this format
            ...
            trx.description = f"Top-Up to {trx.recipient_name}"
            return trx
        return None

    def _extract_transfer(self, email: str) -> Optional[TransactionData]:
        """
