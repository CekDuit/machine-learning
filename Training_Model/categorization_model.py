...
def preprocess_transaction_data(train_df, test_df):
    ...
    tfidf_vectorizer_notes = keras.layers.TextVectorization(max_tokens=150, output_mode='tf-idf')
    ...
    tfidf_vectorizer_merchant = keras.layers.TextVectorization(max_tokens=150, output_mode='tf-idf')
    ...
    lookup_payment_method = keras.layers.StringLookup()
    lookup_transaction_type = keras.layers.StringLookup()
...
def create_model(input_dim, output_dim, category_encoding_layer, tfidf_vectorizer_notes, tfidf_vectorizer_merchant):
    model = keras.models.Sequential([
        keras.layers.Dense(1024, kernel_regularizer=keras.regularizers.l1_l2(l1=1e-8, l2=1e-7)),
        keras.layers.BatchNormalization(momentum=0.9),
        keras.layers.LeakyReLU(negative_slope=0.1),
        keras.layers.Dropout(0.1),
        keras.layers.Dense(512, kernel_regularizer=keras.regularizers.l1_l2(l1=1e-8, l2=1e-7)),
        keras.layers.BatchNormalization(momentum=0.9),
        keras.layers.LeakyReLU(negative_slope=0.1),
        keras.layers.Dropout(0.1),
        keras.layers.Dense(256, kernel_regularizer=keras.regularizers.l1_l2(l1=1e-8, l2=1e-7)),
        keras.layers.BatchNormalization(momentum=0.9),
        keras.layers.LeakyReLU(alpha=0.1),
        keras.layers.Dropout(0.1),
        keras.layers.Dense(128, kernel_regularizer=keras.regularizers.l1_l2(l1=1e-8, l2=1e-7)),
        keras.layers.BatchNormalization(momentum=0.9),
        keras.layers.LeakyReLU(alpha=0.1),
        keras.layers.Dropout(0.1),
...
def train_and_evaluate(training_data_path, testing_data_path):
    ...
    y_train = np.array(le_category.fit_transform(train_df['Category']))
    ...
    y_test_onehot = keras.utils.to_categorical(y_test) if y_test.size > 0 else None
